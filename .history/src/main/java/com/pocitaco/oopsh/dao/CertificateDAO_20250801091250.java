...
public class CertificateDAO extends BaseDAO<Certificate, Integer> {
    ...
    public CertificateDAO() {
        super("data/certificates.xml", "certificate");
    }
    ...
    protected Certificate elementToEntity(Element element) {
        ...
        // Check if examTypeId exists
        if (element.getElementsByTagName("examTypeId").getLength() > 0) {
            certificate.setExamTypeId(
                    Integer.parseInt(element.getElementsByTagName("examTypeId").item(0).getTextContent()));
        }
    ...
    protected Element entityToElement(Document doc, Certificate certificate) {
        ...
        examTypeIdElement.setTextContent(String.valueOf(certificate.getExamTypeId()));
    ...
    public List<Certificate> findByUserIdAndExamTypeId(int userId, int examTypeId) {
        lock.readLock().lock();
        try {
            List<Certificate> certificates = new ArrayList<>();
            Document doc = loadDocument();
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                int elementCandidateId = Integer
                        .parseInt(element.getElementsByTagName("candidateId").item(0).getTextContent());
                int elementExamTypeId = Integer
                        .parseInt(element.getElementsByTagName("examTypeId").item(0).getTextContent());
                if (elementCandidateId == userId && elementExamTypeId == examTypeId) {
                    certificates.add(elementToEntity(element));
                }
            }
            return certificates;
