        ...
        if (createdDateStr != null && !createdDateStr.isEmpty()) {
            try {
                ...
                if (createdDateStr.contains(" ")) {
                    ...
                    // If it's just a date, parse as LocalDate and convert to LocalDateTime
                    LocalDate date = LocalDate.parse(createdDateStr, DATE_FORMATTER);
                    schedule.setCreatedDate(date.atStartOfDay());
                }
            } catch (Exception e) {
                // If parsing fails, set current date
                schedule.setCreatedDate(LocalDateTime.now());
            }
        } else {
            // If no createdDate, set current date
            schedule.setCreatedDate(LocalDateTime.now());
        }
