...
public class ExamTypeDAO {
    ...
    private void addExamTypeToDocument(Document doc, Element root, ExamType examType) {
        ...
        examTypeElement.appendChild(createElement(doc, "status", examType.getStatus()));
        examTypeElement.appendChild(createElement(doc, "code", examType.getCode()));
    ...
    private ExamType parseExamTypeFromElement(Element examTypeElement) {
        ...
        String statusStr = getElementText(examTypeElement, "status");
        if (statusStr != null && !statusStr.isEmpty()) {
            examType.setStatus(statusStr);
        } else {
            examType.setStatus("ACTIVE"); // Default status
        }

        String codeStr = getElementText(examTypeElement, "code");
        if (codeStr != null && !codeStr.isEmpty()) {
            examType.setCode(codeStr);
        } else {
            examType.setCode(examType.getName()); // Use name as code if not provided
        }
