...
public class SessionReportDAO extends BaseDAO<SessionReport, Integer> {
    ...
    public SessionReportDAO() {
        super("data/session_reports.xml", "sessionReport");
    }
    ...
    protected SessionReport elementToEntity(Element element) {
        ...
        NodeList reportContentList = element.getElementsByTagName("reportContent");
        if (reportContentList.getLength() > 0) {
            report.setReportContent(reportContentList.item(0).getTextContent());
        } else {
            report.setReportContent("");
        }

        NodeList notesList = element.getElementsByTagName("notes");
        if (notesList.getLength() > 0) {
            report.setNotes(notesList.item(0).getTextContent());
        } else {
            report.setNotes("");
        }
