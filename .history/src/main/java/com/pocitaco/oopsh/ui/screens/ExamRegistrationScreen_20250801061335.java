...
public class ExamRegistrationScreen extends BorderPane {
    ...
    public ExamRegistrationScreen(User user) {
        ...
        logger.userAction(currentUser.getUsername(), "ExamRegistrationScreen", "Screen initialized");
        performanceMonitor.startOperation("ExamRegistrationScreen_Initialization");

        try {
            initializeScreen();
            loadDataAsync();
            updateStatistics();
            logger.info("ExamRegistrationScreen initialized successfully");
        } catch (Exception e) {
            logger.error("Failed to initialize ExamRegistrationScreen", e);
            showError("Failed to initialize screen: " + e.getMessage());
        } finally {
            performanceMonitor.endOperation("ExamRegistrationScreen_Initialization");
        }
    }
    ...
    private void createLoadingOverlay() {
        ...
        loadingIndicator.setStyle("-fx-progress-color: " + MaterialDesignManager.Colors.PRIMARY_COLOR + ";");

        Label loadingLabel = new Label("Loading data...");
        loadingLabel.setFont(MaterialDesignManager.Typography.BODY_MEDIUM);
        loadingLabel.setStyle(MaterialDesignManager.Colors.TEXT_PRIMARY);

        VBox loadingContent = new VBox(16, loadingIndicator, loadingLabel);
        ...
        stackPane.getChildren().addAll(getCenter(), loadingOverlay);
        setCenter(stackPane);
    }
    ...
    private TableView<ExamSchedule> createAvailableExamsTable() {
        ...
        timeCol.setCellValueFactory(cellData -> {
            ...
            return new SimpleStringProperty(timeSlot != null ? timeSlot.toString() : "N/A");
        });
    ...
    private void loadDataAsync() {
        ...
        Task<Void> loadTask = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                performanceMonitor.startOperation("ExamRegistrationScreen_LoadData");
                try {
                    loadAvailableExams();
                    loadMyRegistrations();
                    loadExamTypes();
                    updateStatistics();
                    logger.info("Data loaded successfully for ExamRegistrationScreen");
                } finally {
                    performanceMonitor.endOperation("ExamRegistrationScreen_LoadData");
                }
                return null;
            }
        };
        ...
        loadTask.setOnSucceeded(event -> {
            Platform.runLater(() -> {
                showLoading(false);
                logger.info("Async data loading completed successfully");
            });
        });
        ...
        loadTask.setOnFailed(event -> {
            Platform.runLater(() -> {
                ...
                logger.error("Async data loading failed", exception);
    ...
    private void loadAvailableExams() {
        try {
            performanceMonitor.startOperation("LoadAvailableExams");
            logger.database("SELECT", "exam_schedules",
                    "Loading available exams for user: " + currentUser.getUsername());
            ...
            logger.info("Loaded " + availableSchedules.size() + " available exams");
            performanceMonitor.endOperation("LoadAvailableExams");
        } catch (Exception e) {
            logger.error("Error loading available exams", e);
    ...
    private void loadMyRegistrations() {
        try {
            performanceMonitor.startOperation("LoadMyRegistrations");
            logger.database("SELECT", "registrations", "Loading registrations for user: " + currentUser.getUsername());
            ...
            logger.info("Loaded " + registrations.size() + " registrations for user");
            performanceMonitor.endOperation("LoadMyRegistrations");
        } catch (Exception e) {
            logger.error("Error loading registrations", e);
    ...
    private void loadExamTypes() {
        try {
            performanceMonitor.startOperation("LoadExamTypes");
            logger.database("SELECT", "exam_types", "Loading exam types for filters");

            List<ExamType> examTypes = examTypeDAO.getAll();
            examTypeFilter.getItems().clear();
            examTypeFilter.getItems().addAll(examTypes);

            logger.info("Loaded " + examTypes.size() + " exam types");
            performanceMonitor.endOperation("LoadExamTypes");
        } catch (Exception e) {
            logger.error("Error loading exam types", e);
    ...
    private void registerForExam(ExamSchedule schedule) {
        ...
        confirmation.setGraphic(MaterialDesignManager.Icons.createInfoIcon());

        try {
            ExamType examType = examTypeDAO.findById(schedule.getExamTypeId()).orElse(null);
            String examName = examType != null ? examType.getName() : "N/A";
            String location = schedule.getLocation() != null ? schedule.getLocation() : "N/A";
            String timeSlot = schedule.getTimeSlot() != null ? schedule.getTimeSlot().toString() : "N/A";
        ...
        confirmation.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    showSuccess(
                            "Registration successful! You can view your registration in the 'My Registrations' tab.");
                } catch (Exception e) {
    ...
    private void cancelRegistration(Registration registration) {
        ...
        confirmation.setGraphic(MaterialDesignManager.Icons.createWarningIcon());
        ...
        try {
            ...
            confirmation.setContentText("Are you sure you want to cancel this registration?\n\n" +
                    "Registration ID: REG-" + registration.getId() + "\n" +
                    "Exam: " + examName + "\n" +
        ...
        confirmation.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    loadMyRegistrations();
                    loadAvailableExams(); // Refresh to update status
                    showSuccess("Registration cancelled successfully! You can register for another exam if needed.");
                } catch (Exception e) {
    ...
    private void updateStatistics() {
        ...
        // Calculate pending and completed registrations
        long pendingExams = myRegistrationsData.stream()
                .filter(reg -> "PENDING".equals(reg.getStatus()) || "REGISTERED".equals(reg.getStatus()))
                .count();
        long completedExams = myRegistrationsData.stream()
        ...
        Label pendingValueLabel = (Label) ((VBox) pendingExamsLabel.getGraphic()).getChildren().get(0);
        pendingValueLabel.setText(String.valueOf(pendingExams));

        Label completedValueLabel = (Label) ((VBox) completedExamsLabel.getGraphic()).getChildren().get(0);
        completedValueLabel.setText(String.valueOf(completedExams));
    }
    ...
    private void showSuccess(String message) {
        ...
        alert.setGraphic(MaterialDesignManager.Icons.createCheckCircleIcon());
        alert.showAndWait();
    }
    ...
    private void showError(String message) {
        ...
        alert.setHeaderText(null);
        alert.setContentText(message);
        alert.setGraphic(MaterialDesignManager.Icons.createErrorIcon());
        alert.showAndWait();
    }

    // Navigation methods with proper logging and performance monitoring
    private void navigateToDashboard() {
        try {
            logger.userAction(currentUser.getUsername(), "Navigation", "Navigate to Candidate Dashboard");
            performanceMonitor.startOperation("NavigateToDashboard");
            
            navigationManager.navigateToCandidateDashboard(currentUser);
            
            performanceMonitor.endOperation("NavigateToDashboard");
        } catch (Exception e) {
            logger.error("Failed to navigate to dashboard", e);
            showError("Cannot navigate to dashboard: " + e.getMessage());
        }
    }

    private void openPracticeTests() {
        try {
            logger.userAction(currentUser.getUsername(), "Navigation", "Open Practice Tests");
            performanceMonitor.startOperation("OpenPracticeTests");
            
            navigationManager.navigateToPracticeTests(currentUser);
            
            performanceMonitor.endOperation("OpenPracticeTests");
        } catch (Exception e) {
            logger.error("Failed to open practice tests", e);
            showError("Cannot open practice tests: " + e.getMessage());
        }
    }

    private void openExamResults() {
        try {
            logger.userAction(currentUser.getUsername(), "Navigation", "Open Exam Results");
            performanceMonitor.startOperation("OpenExamResults");
            
            navigationManager.navigateToExamResults(currentUser);
            
            performanceMonitor.endOperation("OpenExamResults");
        } catch (Exception e) {
            logger.error("Failed to open exam results", e);
            showError("Cannot open exam results: " + e.getMessage());
        }
    }

    private void openStudyMaterials() {
        try {
            logger.userAction(currentUser.getUsername(), "Navigation", "Open Study Materials");
            performanceMonitor.startOperation("OpenStudyMaterials");
            
            navigationManager.navigateToStudyMaterials(currentUser);
            
            performanceMonitor.endOperation("OpenStudyMaterials");
        } catch (Exception e) {
            logger.error("Failed to open study materials", e);
            showError("Cannot open study materials: " + e.getMessage());
        }
    }

    private void logout() {
        try {
            logger.userAction(currentUser.getUsername(), "Logout", "User logged out");
            performanceMonitor.startOperation("Logout");
            
            navigationManager.logoutUser();
            
            performanceMonitor.endOperation("Logout");
        } catch (Exception e) {
    ...
    private void viewRegistrationDetails(Registration registration) {
        try {
            ExamType examType = examTypeDAO.findById(registration.getExamTypeId()).orElse(null);
            String examName = examType != null ? examType.getName() : "N/A";
            String examDescription = examType != null ? examType.getDescription() : "N/A";

            Alert detailsDialog = new Alert(Alert.AlertType.INFORMATION);
            detailsDialog.setTitle("Registration Details");
            detailsDialog.setHeaderText("Registration Information");
            detailsDialog.setGraphic(MaterialDesignManager.Icons.createFileDocumentIcon());

            String content = String.format(
                    "Registration ID: REG-%d\n\n" +
                            "Exam Information:\n" +
                            "• Name: %s\n" +
                            "• Description: %s\n" +
                            "• Duration: %d minutes\n" +
                    ...
                    examName,
                    examDescription,
                    examType != null ? examType.getDuration() : 0,
                    examType != null ? examType.getPassingScore() : 0,
                    examType != null ? examType.getFee() : 0,
            ...
            showError("Error loading registration details: " + e.getMessage());
        }
    }
}
