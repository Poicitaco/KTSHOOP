...
public class NavigationManager {
    ...
    public void navigateToPracticeTests() {
        try {
            performanceMonitor.startOperation("NavigateToPracticeTests");
            logger.userAction(currentUser.getUsername(), "Navigation", "Navigate to Practice Tests");

            String cacheKey = "practice_tests_" + currentUser.getId();
            PracticeTestsScreen practiceTestsScreen = (PracticeTestsScreen) screenCache.get(cacheKey);

            if (practiceTestsScreen == null) {
                practiceTestsScreen = new PracticeTestsScreen(currentUser);
                screenCache.put(cacheKey, practiceTestsScreen);
                logger.info("Created new PracticeTestsScreen instance");
            }

            // Create scene and show
            Scene scene = new Scene(practiceTestsScreen, 1200, 800);
            primaryStage.setScene(scene);
            primaryStage.show();
            logger.info("Successfully navigated to Practice Tests");
            performanceMonitor.endOperation("NavigateToPracticeTests");
        } catch (Exception e) {
            logger.error("Error navigating to practice tests", e);
            performanceMonitor.endOperation("NavigateToPracticeTests");
            throw new RuntimeException("Failed to navigate to Practice Tests", e);
        }
    }
    ...
    public void navigateToExamResults() {
        try {
            performanceMonitor.startOperation("NavigateToExamResults");
            logger.userAction(currentUser.getUsername(), "Navigation", "Navigate to Exam Results");

            String cacheKey = "exam_results_" + currentUser.getId();
            ExamResultsScreen examResultsScreen = (ExamResultsScreen) screenCache.get(cacheKey);

            if (examResultsScreen == null) {
                examResultsScreen = new ExamResultsScreen(primaryStage, currentUser);
                screenCache.put(cacheKey, examResultsScreen);
                logger.info("Created new ExamResultsScreen instance");
            }

            // Create scene and show
            Scene scene = examResultsScreen.createScene();
            primaryStage.setScene(scene);
            primaryStage.show();

            logger.info("Successfully navigated to Exam Results");
            performanceMonitor.endOperation("NavigateToExamResults");
        } catch (Exception e) {
            logger.error("Error navigating to exam results", e);
            performanceMonitor.endOperation("NavigateToExamResults");
            throw new RuntimeException("Failed to navigate to Exam Results", e);
        }
    }
    ...
    public void navigateToStudyMaterials() {
        try {
            performanceMonitor.startOperation("NavigateToStudyMaterials");
            logger.userAction(currentUser.getUsername(), "Navigation", "Navigate to Study Materials");

            String cacheKey = "study_materials_" + currentUser.getId();
            StudyMaterialsScreen studyMaterialsScreen = (StudyMaterialsScreen) screenCache.get(cacheKey);

            if (studyMaterialsScreen == null) {
                studyMaterialsScreen = new StudyMaterialsScreen(currentUser);
                screenCache.put(cacheKey, studyMaterialsScreen);
                logger.info("Created new StudyMaterialsScreen instance");
            }

            // Create scene and show
            Scene scene = new Scene(studyMaterialsScreen, 1200, 800);
            primaryStage.setScene(scene);
            primaryStage.show();

            logger.info("Successfully navigated to Study Materials");
            performanceMonitor.endOperation("NavigateToStudyMaterials");
        } catch (Exception e) {
            logger.error("Error navigating to study materials", e);
            performanceMonitor.endOperation("NavigateToStudyMaterials");
            throw new RuntimeException("Failed to navigate to Study Materials", e);
        }
    }
    ...
    public void navigateToPayment() {
        try {
            performanceMonitor.startOperation("NavigateToPayment");
            logger.userAction(currentUser.getUsername(), "Navigation", "Navigate to Payment");

            String cacheKey = "payment_" + currentUser.getId();
            PaymentScreen paymentScreen = (PaymentScreen) screenCache.get(cacheKey);

            if (paymentScreen == null) {
                paymentScreen = new PaymentScreen(currentUser);
                screenCache.put(cacheKey, paymentScreen);
                logger.info("Created new PaymentScreen instance");
            }

            // Create scene and show
            Scene scene = new Scene(paymentScreen, 1200, 800);
            primaryStage.setScene(scene);
            primaryStage.show();

            logger.info("Successfully navigated to Payment");
            performanceMonitor.endOperation("NavigateToPayment");
        } catch (Exception e) {
            logger.error("Error navigating to payment", e);
            performanceMonitor.endOperation("NavigateToPayment");
            throw new RuntimeException("Failed to navigate to Payment", e);
        }
    }
